...
export default function HeaderMenuTaiKhoan({ avatar, hoTen }) {
  ...
  return (
    <div
      ref={wrapperRef}
      className="d-flex userLogin align-items-center"
      onClick={() => {
        setOpen((open) => !open);
      }}
    >
      ...
      {open ? (
        <div className="logout text-dark">
          <NavLink className="link-logout text-nowrap" to="/thongtintaikhoan">
            Tài khoản
          </NavLink>
          <div className="link-logout text-nowrap" onClick={() => dangXuat()}>
            Đăng xuất
          </div>
        </div>
      ) : (
