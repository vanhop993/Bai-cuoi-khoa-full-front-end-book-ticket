        ...
        <>
            <div className='row chi-tiet-noi-dung-phim'>
                <div className="col-3 position-relative d-none d-md-block">
                    <img className='w-100' src={props.chiTietPhim.hinhAnh} alt={props.chiTietPhim.tenPhim} />
                    <div className='border-btn-trailer'>
                        <IoIosPlay className="btn-trailer-play-center" onClick={() => setOpen(true)} />
                    </div>
                </div>
                <div className='col-6 my-auto d-none d-md-block'>
                    <div className='text-light text-shadow'>
                        <p>{moment(props.chiTietPhim.ngayKhoiChieu).format('DD-MM-YYYY')}</p>
                        <h3 className='text-light'>{props.chiTietPhim.tenPhim}</h3>
                        <p><ThoiLuongPhim chiTietPhim={props.chiTietPhim}/> - 0 IMDb - 2D/Digital</p>
                        <p>{props.chiTietPhim.moTa}</p>
                        <button className='btn btn-mua-ve' onClick={() => handleScrollSpy('menuDatVe')}>Mua vé</button>
                    </div>
                </div>
                <div className="col-3 m-auto d-none d-md-block" >
                    ...
                    <div className='StarNumber w-100'>
                        <div className='mx-auto mt-4 w-fit-content'><RenderStar danhGia={props.chiTietPhim.danhGia}/></div>
                    </div>
                </div>
                <iframe className='w-100 d-block d-md-none' src={props.chiTietPhim.trailer} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen/>
            </div>
            <ModalVideo
                channel="youtube"
                autoplay
